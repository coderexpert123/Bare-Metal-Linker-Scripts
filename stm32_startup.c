 ...
 * @attention
 *
 * Copyright (c) 2024 Nishant Singh.
 * All rights reserved.
 *
...
void reset_handler(void) {
 ...
 uint32_t size=&_etext - &_sdata;
 uint32_t *pDst=(uint32_t*)_sdata;//sram 
 uint32_t *pSrc=(uint32_t*)_etext;//flash
 ...
 //inti .bss section to zero in SRAM 
   size=&_ebss - &_sbss;
    for(uint32_t i=0;i<size;i++){
